...
export function TransactionTabContent({
  ...
  const handleUpdateStatus = (
    id: string,
    newStatus: "Selesai" | "Proses" | "Batal",
  ) => {
    ...
    <div className="space-y-8 font-sans select-none pb-16 text-white text-left">
      <div>
        ...
        <p className="text-xs text-[#333333] font-bold uppercase tracking-wider mt-1">
          Laporan Finansial & Sinkronisasi Penjualan
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Card 1 */}
        <div className="bg-white rounded-[24px] border border-[#e6e6e6] p-5 shadow-md flex items-center gap-4 relative overflow-hidden group">
          <div className="bg-white text-black p-3 rounded-xl border border-black">
            <DollarSign className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] text-[#333333] font-black uppercase tracking-wider">
              Omset Selesai
            </span>
            <p className="text-lg font-black text-[#333333] tracking-tight font-mono mt-0.5">
        ...
        <div className="bg-white rounded-[24px] border border-[#e6e6e6] p-5 shadow-md flex items-center gap-4 relative overflow-hidden group">
          ...
          <div>
            ...
            <p className="text-lg font-black text-[#333333] tracking-tight font-mono mt-0.5">
              {filteredTrans.filter((tx) => tx.status === "Proses").length}{" "}
              BARIS
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[40px] shadow-2xl border border-[#E6E6E6] overflow-hidden">
        ...
        <div className="p-6 md:p-8 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-2.5">
            {(["Semua", "Selesai", "Proses", "Batal"] as const).map(
              (status) => (
                <button
                  key={status}
                  ...
                  className={`px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest cursor-pointer transition-transform hover:scale-[1.03] ${
                    filterStatus === status
                      ? "bg-[#1B3551] text-white font-black"
                      : "bg-white text-[#333333] border border-[#E6E6E6] hover:text-[#333333] "
                  }`}
                >
                  {status}
                </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f4f4f4] border-b-1 border-[#E6E6E6] text-xs font-black text-[#333333] uppercase tracking-widest">
                <th className="py-4 px-6 w-32">ID NOTA</th>
              ...
              {filteredTrans.length === 0 ? (
                ...
                filteredTrans.map((tx) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-xs text-[#333333] font-mono font-bold">
                      {tx.date.toUpperCase()}
                    </td>
                    <td
                      className="py-4 px-6 text-xs text-[#333333] max-w-xs truncate font-semibold"
                      title={tx.itemsSummary}
                    >
                      {tx.itemsSummary}
                    </td>
                    <td className="py-4 px-6 font-mono text-xs font-bold text-[#333333]">
