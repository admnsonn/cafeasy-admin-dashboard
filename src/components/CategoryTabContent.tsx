...
export function CategoryTabContent({
  ...
  const handleDeleteCat = (id: string) => {
    if (
      confirm(
        "Hapus kategori ini? Seluruh menu di bawah kategori ini juga akan dinormalisasi.",
      )
    ) {
    ...
    <div className="space-y-8 font-sans select-none pb-16 text-white text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#333333] font-bold uppercase tracking-wider mt-1">
            Klasifikasi & Pengelompokan Menu Cafe
          </p>
        </div>
        {/* <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white rounded-[40px] border border-white/10 p-5 shadow-lg flex items-center justify-between gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {filteredCategories.length === 0 ? (
              ...
              filteredCategories.map((cat) => (
                <div
                  key={cat.id}
                  className="bg-white border-1 border-[#E6E6E6] p-6 rounded-[24px] hover:border-white/30 transition-all shadow-md flex flex-col justify-between group"
                >
                  <div>
                    <div className="flex items-center justify-between mb-3 text-xs">
                      <span className="font-mono text-[#333333] font-bold uppercase tracking-widest">
                        {cat.id}
                      </span>
                      <span className="bg-[#1B3551] text-white font-black px-2 py-2 rounded-full flex items-center gap-1 uppercase tracking-widest text-[9px] leading-none">
                        <Tag size={9} />
                        Kategori
                      </span>
                    </div>
                    <h4 className="font-black text-[#333333] text-lg mb-1 uppercase tracking-tight">
                      {cat.name}
                    </h4>
                  ...
                  <div className="mt-6 pt-3 border-t border-white/5 flex justify-end">
                    <button
                      ...
                      className="text-[#333333] p-1.5 rounded-lg transition-colors cursor-pointer"
        ...
        <div className="bg-[#1B3551] text-white rounded-[32px] border border-white/10 p-6 shadow-md h-fit md:sticky md:top-28">
          ...
          <form onSubmit={handleAddCategory} className="space-y-5">
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-white block mb-1.5">
                Nama Kategori
              </label>
              <input
                type="text"
                ...
                className="w-full bg-white text-[#333333] placeholder:text-[#333333] rounded-lg border border-[#E6E6E6] py-2.5 px-4 text-xs font-medium focus:outline-none"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white text-[#333333] placeholder:text-[#333333] rounded-lg border border-[#E6E6E6] py-2.5 px-4 text-xs font-medium focus:outline-none resize-none"
              />
            </div>

            <button
              type="submit"
              className="w-full py-3 bg-[#1A1A27] text-white font-extrabold text-xs tracking-widest uppercase rounded-full transition-transform cursor-pointer flex items-center justify-center gap-1.5"
            >
