...
export function CustomerTabContent({
  ...
  const handleAddCustomer = (e: React.FormEvent) => {
    ...
    const newCust: Customer = {
      ...
      phone: newPhone,
      email:
        newEmail || `${newName.toLowerCase().replace(/\s+/g, "")}@mail.com`,
      status: newStatus,
    };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#333333] font-bold uppercase tracking-wider mt-1">
            Manajemen Loyalitas & Profil Member
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F4F4F4] border-b-1 border-[#E6E6E6] text-xs font-black text-[#333333] uppercase tracking-widest">
                <th className="py-4 px-6 w-28">ID MEMBER</th>
              ...
              {filteredCust.length === 0 ? (
                ...
                filteredCust.map((c) => (
                  <tr key={c.id} className="hover:bg-white/5 transition-colors">
                    <td className="py-4 px-6 font-mono text-xs text-zinc-500 font-bold">
                      {c.id.toUpperCase()}
                    </td>
                    ...
                    {editingId === c.id ? (
                      <>
                        <td className="py-3 px-6">
                          <input
                            ...
                            className="bg-white border-1 border-[#e6e6e6] text-[#333333] rounded font-bold px-3 py-1.5 text-sm w-36 outline-none focus:border-white"
                          />
                        </td>
                        <td className="py-3 px-6">
                          <input
                            ...
                            className="bg-white border-1 border-[#e6e6e6] text-[#333333] rounded font-mono font-bold px-3 py-1.5 text-sm w-32 outline-none focus:border-white"
                          />
                        </td>
                        <td className="py-3 px-6">
                          <input
                            ...
                            className="bg-white border-1 border-[#e6e6e6] text-[#333333] rounded font-mono font-bold px-3 py-1.5 text-sm w-40 outline-none focus:border-white"
                          />
                        </td>
                        <td className="py-3 px-6 text-center">
                          <select
                            ...
                            className="bg-[#1B3551] border-1 border-[#e6e6e6] text-white rounded text-xs py-1.5 px-3 font-semibold cursor-pointer focus:outline-none"
                          >
                            <option value="Aktif" className="bg-[#1B3551]">
                              Aktif
                            </option>
                            <option value="Nonaktif" className="bg-[#1B3551]">
                              Nonaktif
                            </option>
                          </select>
                        </td>
                      ...
                      <>
                        ...
                        <td className="py-4 px-6 text-center animate-fade-in">
                          <span
                            className={`inline-flex px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full ${
                              c.status === "Aktif"
